...
if (isset($_POST['add_chef_submit'])) {
    ...
    if (empty($name) || empty($email) || empty($pass) || empty($speciality) || empty($restaurant_name) || empty($salary)) {
        ...
        $resultCheck = mysqli_num_rows($result);
        if ($resultCheck > 0) {
            header("Location:employee.php?title=Employees&error=Chef Already exsist");
            exit();
        } else {
            ...
            header("Location:employee.php?title=Employees&msg=chef Added ");
...
if (isset($_POST['update_chef_submit'])) {
    ...
    header("Location:employee.php?title=Employees&msg=Chef Data updtaed sucessfully ");
...
if (isset($_GET['del_id'])) {
    ...
    header("Location:employee.php?title=Employees&msg=Chef Deleted sucessfully ");
